...
export type RateLimitStore = {
  ...
  set: (key: string, value: AuthRateLimitRecord, ttlSeconds: number) => Promise<void>;
  del: (key: string) => Promise<void>;
};

const memoryStore = new Map<string, AuthRateLimitRecord>();
...
function createUpstashStore(config: { url: string; token: string }): RateLimitStore {
  return {
    get: async (key) => {
      ...
      if (!raw) return null;
      return JSON.parse(raw) as AuthRateLimitRecord;
    },
...
export function getUpdatedRateLimitRecord(
  existing: AuthRateLimitRecord | null,
  now: number,
  windowMs: number,
  maxAttempts: number,
  blockMs: number,
...
export async function readRateLimitRecord(
