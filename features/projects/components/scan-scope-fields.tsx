...
import { useGithubRepositoryFiles } from "@/features/projects/hooks/use-projects";
import { MAX_PROJECT_WATCH_FILES } from "@/features/projects/model";
import { cn } from "@/lib/utils";

type ScanMode = "full" | "selected";

type ScanScopeFieldsProps = {
  fullName: string;
  branch: string;
  scanMode: ScanMode;
  files: string[];
...
export function ScanScopeFields({
  ...
  const searchEnabled = scanMode === "selected" && Boolean(fullName);
  const filesQuery = useGithubRepositoryFiles(
  ...
  const addFile = (path: string) => {
    if (selected.has(path) || files.length >= MAX_PROJECT_WATCH_FILES) return;
    ...
    <div className="space-y-4">
      <fieldset className="space-y-2">
        ...
        >
          ...
          <span className="min-w-0">
            <span className="block text-sm font-medium">Entire GitHub repository</span>
        ...
        >
          ...
          />
          <span className="min-w-0">
            <span className="block text-sm font-medium">Selected files</span>
            <span className="mt-0.5 block text-xs text-muted-foreground">
              Search and pick files to monitor — including custom names. Only those files are scanned.
            </span>
          </span>
        </label>
      </fieldset>
      ...
      {scanMode === "selected" ? (
        <div className="space-y-3 rounded-lg border bg-muted/20 p-3">
          {files.length ? (
            <ul className="flex flex-wrap gap-1.5" aria-label="Files to monitor">
              {files.map((path) => (
                <li key={path}>
            ...
            <p className="text-xs text-muted-foreground">
              Search by name (package.json, bom.yaml, or a company-specific file) and add each path you
              want monitored.
            </p>
          )}
                ...
                return (
                  <li key={path}>
                    <Button
                      ...
                      disabled={disabled || added || files.length >= MAX_PROJECT_WATCH_FILES}
          ...
          <p className="text-xs text-muted-foreground">
            {files.length}/{MAX_PROJECT_WATCH_FILES} files selected. This list is saved on the project and used
            for every scan, including scheduled scans.
          </p>
        </div>
      ) : null}
    </div>
